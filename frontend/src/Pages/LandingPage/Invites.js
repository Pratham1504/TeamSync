...
const Invites = () => {
    ...
    const handleAccept = async (Orgid , Inviteid) =>{
        ...
        await fetch(`organisation/${Orgid}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
        ...
        await fetch(`user/${JSON.parse(localStorage.getItem('user'))._id}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ "orgs": [...orgs, newOrgtoPush] })
        })
                        ...
                        {invite && invite.map((invites) => (
                            <div className="invite-details" style={{ fontFamily: "Verdana" }}>
                                ...
                                <div style={{ display: "flex", alignItems: "space-between", minWidth: "300px", justifyContent: "center", marginTop: "3%" }}>
                                    ...
                                    <button type="submit" value="Submit" style={{ marginRight: "5%", backgroundColor: "blue", color: "white" }} onClick={() => handleAccept(invites.OrgId,invites._id)}>Accept</button>
                                    <button type="reset" value="Reset" style={{ marginLeft: "5%" }} onClick={() => handleDelete(invites.OrgId,invites._id)} >Delete</button>
                                </div>
                            </div>
                        ))}
                    </div>
